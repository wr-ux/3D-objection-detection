...
class achelous(object):
    _defaults = {
        ...
        # ---------------------------------------------------------------------#
        #   Debug: dump mask/radar tensor info when True
        # ---------------------------------------------------------------------#
        "dump_tensors"      : False,
        # ---------------------------------------------------------------------#
        #   输入图片的大小，必须为32的倍数。
    ...
    def generate(self, onnx=False):
        ...
        # Handle deploy/reparameterized checkpoints by switching model first.
        is_deploy_ckpt = any('reparam_conv' in k for k in state.keys())
        if is_deploy_ckpt:
        ...
        # Shape-safe load to avoid hard failure when heads differ.
        model_dict = self.net.state_dict()
        load_keys, no_load_keys, temp_dict = [], [], {}
        for k, v in state.items():
            if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
                temp_dict[k] = v
                load_keys.append(k)
            else:
                no_load_keys.append(k)
        model_dict.update(temp_dict)
        self.net.load_state_dict(model_dict)
        if self.dump_tensors:
            print(f"[weights] loaded={len(load_keys)} skipped={len(no_load_keys)}")
    ...
    def detect_image(self, image, image_id, crop=False, count=False, export_all=False):
        ...
        radar_data_raw = np.load(radar_path)['arr_0']
        if self.dump_tensors:
            print(f"[radar] path={radar_path}")
            print(f"[radar] shape={radar_data_raw.shape} dtype={radar_data_raw.dtype}")
        radar_data = torch.from_numpy(preprocess_input_radar(radar_data_raw)).type(torch.FloatTensor).unsqueeze(0)

        # -------------------------------- 点云读取(可选) ---------------------------------- #
        align_radar_pc_features = None
        align_radar_pc_labels = None
        radar_pc_u = None
        radar_pc_v = None
        radar_pc_power = None
        if self.is_radar_seg:
            radar_pc_csv = os.path.join(self.radar_pc_root, image_id + '.csv')
            if not os.path.exists(radar_pc_csv):
                raise FileNotFoundError(f"Radar point cloud CSV not found: {radar_pc_csv}")

            radar_pc_file = pd.read_csv(radar_pc_csv, index_col=0)
            radar_pc_features = radar_pc_file[self.radar_pc_seg_features]
            radar_pc_labels = radar_pc_file[self.radar_pc_seg_labels]
            if self.dump_tensors:
                print(f"Raw Radar shape: {radar_pc_features.shape}")
            ...
            if self.dump_tensors:
                print(f"[radar_pc] csv={radar_pc_csv}")
                print(f"[radar_pc] aligned_features={align_radar_pc_features.shape} dtype={align_radar_pc_features.dtype}")
                print(f"[radar_pc] aligned_labels={align_radar_pc_labels.shape} dtype={align_radar_pc_labels.dtype}")
        ...
        with torch.no_grad():
            ...
            if self.dump_tensors:
                ship_prob = output_seg[..., 8] if output_seg.shape[-1] > 8 else None
                print(f"[mask] prob shape={output_seg.shape} dtype={output_seg.dtype}")
                if ship_prob is not None:
                    print(f"[mask] ship_prob shape={ship_prob.shape} dtype={ship_prob.dtype} min={ship_prob.min():.6f} max={ship_prob.max():.6f}")
            # ---------------------------------------------------#
            #   取出每一个像素点的种类
            # ---------------------------------------------------#
            ...
            if self.dump_tensors:
                ship_mask = (output_seg == 8).astype(np.uint8)
                print(f"[mask] ship_mask shape={ship_mask.shape} dtype={ship_mask.dtype} unique={np.unique(ship_mask)}")
                print(f"Mask shape: {ship_mask.shape}")
                os.makedirs(self.export_path, exist_ok=True)
                np.save(os.path.join(self.export_path, f"{image_id}_ship_prob.npy"), ship_prob)
                np.save(os.path.join(self.export_path, f"{image_id}_ship_mask.npy"), ship_mask)
        ...
        for i, c in list(enumerate(top_label)):
            ...
            label_size = draw.textsize(label, font) if font is not None else (0, 0)
