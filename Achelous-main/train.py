...
if __name__ == "__main__":
    ...
    if is_radar_pc_seg:
        model = Achelous(resolution=input_shape[0], num_det=num_classes, num_seg=num_classes_seg, phi=phi,
                         backbone=backbone, neck=neck, nano_head=lightweight, pc_seg=pc_seg_model,
                                                 pc_channels=radar_pc_channels, pc_classes=radar_pc_classes, spp=spp).to(device)
    else:
    ...
    if model_path != '':
        ...
        # ------------------------------------------------------#
        #   兼容deploy/reparameterized权重：若发现reparam_conv键，先将当前模型重参数化
        # ------------------------------------------------------#
        is_deploy_ckpt = any('reparam_conv' in k for k in pretrained_dict.keys())
        if is_deploy_ckpt:
            if local_rank == 0:
                print('Detected deploy checkpoint, reparameterizing model before loading.')
